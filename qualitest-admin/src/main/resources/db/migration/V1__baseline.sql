...
(
    ...
    `template_content`      text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci          NOT NULL COMMENT '模板正文',
    `builtin_status`        tinyint                                                        NOT NULL DEFAULT 0 COMMENT '内置状态（0自定义 1内置）',
    `enable_status`         tinyint                                                        NOT NULL DEFAULT 1 COMMENT '启用状态（0禁用 1启用）',
    `sort_num`              int                                                            NOT NULL DEFAULT 0 COMMENT '排序',
    `remark`                varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci  NULL     DEFAULT NULL COMMENT '备注',
...
-- 平台内置造流快捷芯片（template_scope=platform）。
-- 子流编排区分两种意图：
--   「管理端/客户端登录子流」= 业务主流里用 Subflow 挂已有登录流；
--   「搭建管理端/客户端登录子流」= 当前流本身是对应端登录子流，空画布生成探活再登录（禁止只插 Subflow）。
-- 正文约定：按端拆分素材入口；头名/头值跟 Profile（Bearer / Session / 自定义头）；extract 跟 designHints+托管头占位符，不写死 JsonPath 样板。
-- ----------------------------
INSERT INTO `ai_prompt_template` VALUES (2070000000000000101, 'platform', NULL, 'test_flow_design', '从零搭建', '空画布或仅有 start', '搭建「{流程名称}」：\n1. {步骤描述}\n2. {步骤描述}\n\n需登录：业务主流用 Subflow 挂对应端已有登录流（管理端 adminAuth / 客户端 clientAuth）；若本流就是登录子流，按端改用「搭建管理端登录子流」或「搭建客户端登录子流」。各步骤后加断言。须按单元调用对应 submit_* 工具（如 submit_http_node / submit_edge，op=add|update）。', 1, 1, 101, '从零搭建', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000103, 'platform', NULL, 'test_flow_design', '预期业务拒绝', '失败路径，关业务 Code', '搭建「{流程名称}」失败路径：\n1. 绑定「{接口}」\n2. successCheck.mode=off\n3. 断言 http.body.code ≠ 成功码，或 msg 含「{拒绝文案}」\n\n测值用场景给定账号字面量。', 1, 1, 102, '从零搭建', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000201, 'platform', NULL, 'test_flow_design', '末尾追加步骤', '画布已有节点', '在流末尾追加：{新步骤描述}。仅追加；现有节点保持不动。', 1, 1, 201, '扩展现有流', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000501, 'platform', NULL, 'test_flow_design', 'Run 失败修复', '@Run 或步骤名', '@{Run:最近一次失败Run} 或步骤「{nodeName}」失败。分析参数、断言、extract 并修复，只修失败相关节点。', 1, 1, 301, 'Run 失败修复', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000601, 'platform', NULL, 'test_flow_design', '管理端登录子流', '业务主流挂管理端登录', '【业务主流用】在 Start 后挂管理端登录 Subflow，复用已有登录流。按下列步骤调工具（每次 1 个）：\n1) list_subflow_templates：选管理端登录流（名含「登录」/Bearer/adminAuth；优先模板种子流）\n2) submit_subflow_node op=add（短名如 n_admin_login）：填真实 subflowId；versionPolicy 可 latest\n3) submit_edge：把子流接到 Start（或原第一跳）与后续业务之间\n\n凭证入口 adminAuth：后续业务靠 Profile 托管头（Bearer 用 {{asset.adminAuth.token}}；Session 跟 cookieName/Cookie）。后续 HTTP 靠托管头复用凭证。\n若当前流本身是管理端登录子流、要生成探活再登录节点：改用芯片「搭建管理端登录子流」。', 1, 1, 401, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000602, 'platform', NULL, 'test_flow_design', '客户端登录子流', '业务主流挂客户端登录', '【业务主流用】在 Start 后挂客户端登录 Subflow，复用已有登录流。按下列步骤调工具（每次 1 个）：\n1) list_subflow_templates：选客户端登录流（名含「登录」/客户端/clientAuth；优先模板种子流）\n2) submit_subflow_node op=add（短名如 n_client_login）：填真实 subflowId；versionPolicy 可 latest\n3) submit_edge：把子流接到 Start（或原第一跳）与后续业务之间\n\n凭证入口 clientAuth：后续业务靠 Profile 托管头（Bearer 用 {{asset.clientAuth.token}}；Session 跟 cookieName/Cookie）。双端同图须两套子流、两套凭证键；后续 HTTP 靠托管头复用凭证。\n若当前流本身是客户端登录子流、要生成探活再登录节点：改用芯片「搭建客户端登录子流」。', 1, 1, 402, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000603, 'platform', NULL, 'test_flow_design', '搭建管理端登录子流', '本流即管理端登录子流时用', '【管理端登录子流·本流用】空画布或仅有 start：生成探活再登录图。固定 adminAuth；本流直接造节点（不用 Subflow）。按下列步骤调工具。\n\n先只读：get_graph_summary → list_project_auth_profiles（认准管理端 Profile 托管头）→ list_asset_variables（无 adminAuth 口令则 upsert_asset_variables）→ search_apis + get_api_details 绑定本项目管理端登录口与探活口（常见 /login、/getInfo）。\n\n固定短名（同会话复用）：cond_token、probe_http、cond_alive、login_http；边 e_token_if、e_token_else、e_probe、e_alive_else。\n\n按序每次恰好 1 个工具（工具名仅下列 submit_*）：\n1) submit_condition_node op=add id=cond_token name=凭证是否存在：IF left=asset.adminAuth.<凭证字段> operator=exists（字段名跟 Profile 托管头占位符，常见 token；Session 可能是 jsessionId）；ELSE 无 conditions；出口只用 submit_edge。\n2) submit_http_node op=add id=probe_http name=探活：callMode=project，绑定探活口；statusCheck.mode=whitelist values=[200,401]；successCheck.mode=off；extracts=[]。\n3) submit_condition_node op=add id=cond_alive name=凭证是否有效：IF left=http.status operator=eq right=200（此 IF 为结束分支，无出边）；ELSE 无 conditions。\n4) submit_http_node op=add id=login_http name=登录：callMode=project，绑定登录口；登录口免鉴权；requestValueOverrides.bodyExample 用 {{asset.adminAuth.username}}/{{asset.adminAuth.password}}；extracts 一条：scope=asset entryKey=adminAuth fieldPath 与托管头一致；from/expr 优先 designHints/响应 schema 与 Profile.headerValueTemplate（Bearer 常见 from=body expr=$.token；Session 用 from=setCookie）。\n5) submit_edge 四条：e_token_if cond_token→probe_http label=if；e_token_else cond_token→login_http label=else；e_probe probe_http→cond_alive；e_alive_else cond_alive→login_http label=else。探活成功 IF 无出边。\n\n拓扑核对：无凭证→直达登录；有凭证→探活→200 结束 / 非200→登录。修已有图用 op=update 或 submit_delete。只含管理端登录探活链。', 1, 1, 403, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000604, 'platform', NULL, 'test_flow_design', '搭建客户端登录子流', '本流即客户端登录子流时用', '【客户端登录子流·本流用】空画布或仅有 start：生成探活再登录图。固定 clientAuth；本流直接造节点（不用 Subflow）。按下列步骤调工具。\n\n先只读：get_graph_summary → list_project_auth_profiles（认准客户端 Profile 托管头）→ list_asset_variables（无 clientAuth 口令则 upsert_asset_variables，字段 mobile/password）→ search_apis + get_api_details 绑定本项目客户端登录口与探活口（常见 /api/account/auth/login、/api/account/auth/profile）。\n\n固定短名（同会话复用）：cond_token、probe_http、cond_alive、login_http；边 e_token_if、e_token_else、e_probe、e_alive_else。\n\n按序每次恰好 1 个工具（工具名仅下列 submit_*）：\n1) submit_condition_node op=add id=cond_token name=凭证是否存在：IF left=asset.clientAuth.<凭证字段> operator=exists（字段名跟 Profile 托管头占位符，常见 token）；ELSE 无 conditions；出口只用 submit_edge。\n2) submit_http_node op=add id=probe_http name=探活：callMode=project，绑定探活口；statusCheck.mode=whitelist values=[200,401]；successCheck.mode=off；extracts=[]。\n3) submit_condition_node op=add id=cond_alive name=凭证是否有效：IF left=http.status operator=eq right=200（此 IF 为结束分支，无出边）；ELSE 无 conditions。\n4) submit_http_node op=add id=login_http name=登录：callMode=project，绑定登录口；登录口免鉴权；requestValueOverrides.bodyExample 用 {{asset.clientAuth.mobile}}/{{asset.clientAuth.password}}；extracts 一条：scope=asset entryKey=clientAuth fieldPath 与托管头一致；from/expr 优先 designHints/响应 schema 与 Profile.headerValueTemplate（Bearer 常见 from=body expr=$.data.token）。\n5) submit_edge 四条：e_token_if cond_token→probe_http label=if；e_token_else cond_token→login_http label=else；e_probe probe_http→cond_alive；e_alive_else cond_alive→login_http label=else。探活成功 IF 无出边。\n\n拓扑核对：无凭证→直达登录；有凭证→探活→200 结束 / 非200→登录。修已有图用 op=update 或 submit_delete。只含客户端登录探活链。', 1, 1, 404, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000002103, 'platform', NULL, 'test_api_design', '测值用素材占位符', '{{asset.*}} 默认测值', '为缺省测值的参数补测值：优先 {{asset.key.field}}，环境相关用 {{env.*}}。密码/Token 用占位符。', 1, 1, 101, '补测值', 0, '2026-07-22 13:51:11', NULL);
...
(
    ...
    `mcp_auto_write_enabled` tinyint                                                   NOT NULL DEFAULT 0 COMMENT '允许MCP自动写流（0否 1是）',
    `mcp_autorun_enabled` tinyint                                                      NOT NULL DEFAULT 0 COMMENT '允许MCP自动跑流（0否 1是）',
