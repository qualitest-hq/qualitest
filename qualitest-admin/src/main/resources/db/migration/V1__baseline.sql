...
-- ----------------------------
INSERT INTO `ai_prompt_template` VALUES (2070000000000000101, 'platform', NULL, 'test_flow_design', '从零搭建', '空画布或仅有 start', '搭建「{流程名称}」：\n1. {步骤描述}\n2. {步骤描述}\n\n需登录：业务主流用 Subflow 挂对应端已有登录流（管理端 adminAuth / 客户端 clientAuth），勿在主流重搭登录 HTTP；若本流就是登录子流，按端改用「搭建管理端登录子流」或「搭建客户端登录子流」。各步骤后加断言。须按单元调用对应 submit_* 工具（如 submit_add_http_node）。', 1, 1, 101, '从零搭建', 0, '2026-07-04 21:37:19', NULL);
...
INSERT INTO `ai_prompt_template` VALUES (2070000000000000601, 'platform', NULL, 'test_flow_design', '管理端登录子流', '业务主流挂管理端登录', '【业务主流用】在 Start 后增加 Subflow，引用项目里已有的管理端登录流「{子流名}」。不要在本图画 Condition/登录 HTTP，也不要复述本提示词了事。\n凭证入口 adminAuth：Bearer 后续业务用 {{asset.adminAuth.token}}；Session 跟 Profile cookieName/Cookie，勿写 Bearer。extract 以登录接口 schema 与托管头为准，勿臆造路径。后续业务 HTTP 勿再登录。\n若当前流本身是管理端登录子流、要生成探活再登录节点，请改用芯片「搭建管理端登录子流」。须按单元调用对应 submit_* 工具（如 submit_add_http_node）。', 1, 1, 401, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000602, 'platform', NULL, 'test_flow_design', '客户端登录子流', '业务主流挂客户端登录', '【业务主流用】在 Start 后增加 Subflow，引用项目里已有的客户端登录流「{子流名}」。不要在本图画 Condition/登录 HTTP，也不要复述本提示词了事。\n凭证入口 clientAuth：Bearer 后续业务用 {{asset.clientAuth.token}}；Session 跟 Profile cookieName/Cookie，勿写 Bearer。extract 以登录接口 schema 与托管头为准，勿臆造路径。双端同图须两套子流，禁止覆盖同一 asset.*.token；后续业务 HTTP 勿再登录。\n若当前流本身是客户端登录子流、要生成探活再登录节点，请改用芯片「搭建客户端登录子流」。须按单元调用对应 submit_* 工具（如 submit_add_http_node）。', 1, 1, 402, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000603, 'platform', NULL, 'test_flow_design', '搭建管理端登录子流', '本流即管理端登录子流时用', '【管理端登录子流本流用】空画布或仅有 start：直接生成管理端探活再登录节点图。固定凭证入口 adminAuth；禁止插 Subflow，禁止只复述本提示词，禁止改用 clientAuth。\n拓扑：Condition(adminAuth 凭证 exists)→管理端探活 HTTP→Condition(http.status=200)；探活成功 IF 不要 target/出边（无出边即结束），ELSE→管理端登录 HTTP。\n硬规则：\n1) 节点/边可用短名 id（如 n_cred、n_probe、n_login），同会话复用同一短名；出边用 submit_add_edge（短名与节点一致），不要写 branches.target；\n2) 登录口 authMode=none，口令 {{asset.adminAuth.*}}；先 search/get_api_details 绑定本项目管理端登录口与探活口；\n3) extract 的 scope/字段必须与管理端 Profile 托管头占位符一致（如 {{asset.adminAuth.data}} 或 {{asset.adminAuth.token}}），勿默认 flow.adminToken；Condition exists 左值与 extract 写入键同一条；\n4) 头名/头值跟管理端 Profile（Bearer→Authorization、Session→Cookie、自定义头如 token 勿改成 Bearer）；JsonPath 优先 designHints/suggestedExtracts/响应 schema，勿臆造 $.token；\n5) 探活 successCheck.mode=off；勿掺业务/客户端步骤；修图用 submit_update_*_node，禁止再 add 一套探活链。\n须按单元调用对应 submit_* 工具（如 submit_add_http_node）。', 1, 1, 403, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000604, 'platform', NULL, 'test_flow_design', '搭建客户端登录子流', '本流即客户端登录子流时用', '【客户端登录子流本流用】空画布或仅有 start：直接生成客户端探活再登录节点图。固定凭证入口 clientAuth；禁止插 Subflow，禁止只复述本提示词，禁止改用 adminAuth。\n拓扑：Condition(clientAuth 凭证 exists)→客户端探活 HTTP→Condition(http.status=200)；探活成功 IF 不要 target/出边（无出边即结束），ELSE→客户端登录 HTTP。\n硬规则：\n1) 节点/边可用短名 id（如 n_cred、n_probe、n_login），同会话复用同一短名；出边用 submit_add_edge（短名与节点一致），不要写 branches.target；\n2) 登录口 authMode=none，口令 {{asset.clientAuth.*}}；先 search/get_api_details 绑定本项目客户端登录口与探活口；\n3) extract 的 scope/字段必须与客户端 Profile 托管头占位符一致（如 {{asset.clientAuth.data}} 或 {{asset.clientAuth.token}}），勿默认 flow.token；Condition exists 左值与 extract 写入键同一条；\n4) 头名/头值跟客户端 Profile（Bearer→Authorization、Session→Cookie、自定义头如 token 勿改成 Bearer）；JsonPath 优先 designHints/suggestedExtracts/响应 schema，勿臆造路径；\n5) 探活 successCheck.mode=off；勿掺业务/管理端步骤；修图用 submit_update_*_node，禁止再 add 一套探活链。\n须按单元调用对应 submit_* 工具（如 submit_add_http_node）。', 1, 1, 404, '子流编排', 0, '2026-07-04 21:37:19', NULL);
