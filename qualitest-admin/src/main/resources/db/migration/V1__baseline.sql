...
INSERT INTO `ai_prompt_template` VALUES (2070000000000000601, 'platform', NULL, 'test_flow_design', '插入登录子流', 'Start 后挂子流', '在 Start 后插入登录子流「{子流名}」。托管头用 {{asset.clientAuth.token}} 或 {{asset.adminAuth.token}}；后续业务 HTTP 勿再登录。', 1, 1, 401, '子流编排', 0, '2026-07-04 21:37:19', NULL);
...
(
    ...
    `template_prompts`         json                                                          NULL COMMENT '预制AI提示词JSON数组；勾选时种子为项目级ai_prompt_template',
    `builtin_status`           tinyint                                                       NOT NULL DEFAULT 0 COMMENT '内置状态（0自定义 1内置）',
    `enable_status`            tinyint                                                       NOT NULL DEFAULT 1 COMMENT '启用状态（0禁用 1启用）',
    `sort_num`                 int                                                           NOT NULL DEFAULT 0 COMMENT '排序',
    `remark`                   varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL     DEFAULT NULL COMMENT '备注',
