...
INSERT INTO `ai_prompt_template` VALUES (2070000000000000601, 'platform', NULL, 'test_flow_design', '管理端登录子流', '业务主流挂管理端登录', '【业务主流用】在 Start 后挂管理端登录 Subflow，复用已有登录流；不要在本图画 Condition/登录 HTTP，不要只复述本提示词。\n\n步骤（每次 1 个工具）：\n1) list_subflow_templates：选管理端登录流（名含「登录」/Bearer/adminAuth；优先模板种子流）\n2) submit_subflow_node op=add（短名如 n_admin_login）：填真实 subflowId；versionPolicy 可 latest\n3) submit_edge：把子流接到 Start（或原第一跳）与后续业务之间\n\n凭证入口 adminAuth：后续业务靠 Profile 托管头（Bearer 用 {{asset.adminAuth.token}}；Session 跟 cookieName/Cookie）。本图后续 HTTP 禁止再插登录。\n若当前流本身是管理端登录子流、要生成探活再登录节点：改用芯片「搭建管理端登录子流」。', 1, 1, 401, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000602, 'platform', NULL, 'test_flow_design', '客户端登录子流', '业务主流挂客户端登录', '【业务主流用】在 Start 后挂客户端登录 Subflow，复用已有登录流；不要在本图画 Condition/登录 HTTP，不要只复述本提示词。\n\n步骤（每次 1 个工具）：\n1) list_subflow_templates：选客户端登录流（名含「登录」/客户端/clientAuth；优先模板种子流）\n2) submit_subflow_node op=add（短名如 n_client_login）：填真实 subflowId；versionPolicy 可 latest\n3) submit_edge：把子流接到 Start（或原第一跳）与后续业务之间\n\n凭证入口 clientAuth：后续业务靠 Profile 托管头（Bearer 用 {{asset.clientAuth.token}}；Session 跟 cookieName/Cookie）。双端同图须两套子流，禁止覆盖同一 asset.*.token；后续 HTTP 禁止再插登录。\n若当前流本身是客户端登录子流、要生成探活再登录节点：改用芯片「搭建客户端登录子流」。', 1, 1, 402, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000603, 'platform', NULL, 'test_flow_design', '搭建管理端登录子流', '本流即管理端登录子流时用', '【管理端登录子流·本流用】空画布或仅有 start：生成探活再登录图。固定 adminAuth；禁止 Subflow；禁止只复述；禁止改用 clientAuth。\n\n先只读：get_graph_summary → list_project_auth_profiles（认准管理端 Profile 托管头）→ list_asset_variables（无 adminAuth 口令则 upsert_asset_variables）→ search_apis + get_api_details 绑定本项目管理端登录口与探活口（常见 /login、/getInfo）。\n\n固定短名（同会话复用）：cond_token、probe_http、cond_alive、login_http；边 e_token_if、e_token_else、e_probe、e_alive_else。\n\n按序每次恰好 1 个工具（工具名必须如下，勿臆造 submit_add_*）：\n1) submit_condition_node op=add id=cond_token name=凭证是否存在：IF left=asset.adminAuth.<凭证字段> operator=exists（字段名跟 Profile 托管头占位符，常见 token；Session 可能是 jsessionId）；ELSE 不要写 conditions。禁止写 branches.target。\n2) submit_http_node op=add id=probe_http name=探活：callMode=project，绑定探活口；statusCheck.mode=whitelist values=[200,401]；successCheck.mode=off；extracts=[]。\n3) submit_condition_node op=add id=cond_alive name=凭证是否有效：IF left=http.status operator=eq right=200（此 IF 为结束分支，后面不要出边）；ELSE 不要写 conditions。\n4) submit_http_node op=add id=login_http name=登录：callMode=project，绑定登录口；登录口免鉴权；requestValueOverrides.bodyExample 用 {{asset.adminAuth.username}}/{{asset.adminAuth.password}}；extracts 一条：scope=asset entryKey=adminAuth fieldPath 与托管头一致；from/expr 优先 designHints/suggestedExtracts/响应 schema（Bearer 常见 from=body expr=$.token；Session 用 from=setCookie）。禁止默认 flow.adminToken。\n5) submit_edge 四条：e_token_if cond_token→probe_http label=if；e_token_else cond_token→login_http label=else；e_probe probe_http→cond_alive；e_alive_else cond_alive→login_http label=else。探活成功 IF 不要边。\n\n拓扑核对：无凭证→直达登录；有凭证→探活→200 结束 / 非200→登录。修已有图用 op=update 或 submit_delete，禁止再 add 一套探活链。勿掺业务或客户端步骤。', 1, 1, 403, '子流编排', 0, '2026-07-04 21:37:19', NULL);
INSERT INTO `ai_prompt_template` VALUES (2070000000000000604, 'platform', NULL, 'test_flow_design', '搭建客户端登录子流', '本流即客户端登录子流时用', '【客户端登录子流·本流用】空画布或仅有 start：生成探活再登录图。固定 clientAuth；禁止 Subflow；禁止只复述；禁止改用 adminAuth。\n\n先只读：get_graph_summary → list_project_auth_profiles（认准客户端 Profile 托管头）→ list_asset_variables（无 clientAuth 口令则 upsert_asset_variables，字段 mobile/password）→ search_apis + get_api_details 绑定本项目客户端登录口与探活口（常见 /api/account/auth/login、/api/account/auth/profile）。\n\n固定短名（同会话复用）：cond_token、probe_http、cond_alive、login_http；边 e_token_if、e_token_else、e_probe、e_alive_else。\n\n按序每次恰好 1 个工具（工具名必须如下，勿臆造 submit_add_*）：\n1) submit_condition_node op=add id=cond_token name=凭证是否存在：IF left=asset.clientAuth.<凭证字段> operator=exists（字段名跟 Profile 托管头占位符，常见 token）；ELSE 不要写 conditions。禁止写 branches.target。\n2) submit_http_node op=add id=probe_http name=探活：callMode=project，绑定探活口；statusCheck.mode=whitelist values=[200,401]；successCheck.mode=off；extracts=[]。\n3) submit_condition_node op=add id=cond_alive name=凭证是否有效：IF left=http.status operator=eq right=200（此 IF 为结束分支，后面不要出边）；ELSE 不要写 conditions。\n4) submit_http_node op=add id=login_http name=登录：callMode=project，绑定登录口；登录口免鉴权；requestValueOverrides.bodyExample 用 {{asset.clientAuth.mobile}}/{{asset.clientAuth.password}}；extracts 一条：scope=asset entryKey=clientAuth fieldPath 与托管头一致；from/expr 优先 designHints/suggestedExtracts/响应 schema（Bearer 常见 from=body expr=$.data.token，勿写成 $.token）。禁止默认 flow.token。\n5) submit_edge 四条：e_token_if cond_token→probe_http label=if；e_token_else cond_token→login_http label=else；e_probe probe_http→cond_alive；e_alive_else cond_alive→login_http label=else。探活成功 IF 不要边。\n\n拓扑核对：无凭证→直达登录；有凭证→探活→200 结束 / 非200→登录。修已有图用 op=update 或 submit_delete，禁止再 add 一套探活链。勿掺业务或管理端步骤。', 1, 1, 404, '子流编排', 0, '2026-07-04 21:37:19', NULL);
