...
模板写入 Profile、登录口免登、**预制登录口**，以及**预制环境（URL + 变量）/ 预制参数（主路径 asset 口令；flow 仅兼容存量）/ 预制测试流 / 预制 AI 提示词**（内置模板种子 `adminAuth` / `clientAuth`，占位环境写成 `http://localhost:8081`；登录流 extracts 派生托管头与 `credentialApi`，**不再**写 `loginHint`；`template_prompts` 种子为项目级造流芯片）；不勾无法创建。**业务 API** 仍须 IDEA 插件上传（手册 **T1.3**）。商城类双端项目建议勾「管理端 Bearer」+「客户端 Bearer」。
