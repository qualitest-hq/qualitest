...
模板写入 Profile、登录口免登、**预制登录口**，以及可选的**预制参数（env/asset；flow 仅兼容存量）/ 预制测试流 / 预制 AI 提示词**（登录流 extracts 派生托管头与 `credentialApi`，**不再**写 `loginHint`；`template_prompts` 种子为项目级造流芯片）；不勾无法创建。**业务 API** 仍须 IDEA 插件上传（手册 **T1.3**）。商城类双端项目建议勾「管理端 Bearer」+「客户端 Bearer」。
