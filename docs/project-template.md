...
菜单「项目模板」：列表、启用、克隆、编辑抽屉、模板画布（`designMode=template`）配登录流。内置行只读，改内容请先克隆。
...
| 形态 | 用途 |
|------|------|
| **精简包（slim）** | 给人 / AI 写的短 JSON：apis、assets、env、credential 等；**不含** flows。导入后展开落库，登录流需再在模板画布配置 |
| **完整包（full）** | 与库内列同形，**可含 flows**；导出默认；另存结果也是完整包 |

导入步骤：

1. 打开「导入」→ 可选 **复制提示词**，贴到任意 AI，按目标系统生成精简 JSON（或多端多份，用 `---` 分隔）。
2. 粘贴或上传 JSON → 预览校验 → 确认导入（可按同名覆盖策略）。
3. 精简包导入后：打开该模板 → 画布配探活 / 登录 / extracts → 保存。
4. 再到测试项目勾选 Apply。
...
| 要做的事 | 走哪里 |
|----------|--------|
| 冷启动鉴权模板 | 复制提示词 → 精简 JSON → 导入 → 画布配流 |
| 复用已跑通项目 | 另存完整模板 |
| 改画布 / 造流 | Web AI 助手 → Staging → 保存；模板画布同理 |
