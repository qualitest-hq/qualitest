...
项目模板**只收鉴权相关口**（登录 / 探活 / 验证码等），**不要**把全量业务 API 塞进模板；业务接口走 IDEA 插件、MCP `import_apis`，或 Web 接口库（手册 **T1.3**；MCP 见 [mcp.md §3.5](./mcp.md)）。
...
3. 有预制登录流时，主流挂该子流即可；业务 API 再经插件 / MCP / Web 入库。
...
| 业务 API 同步 | IDEA 插件、MCP `import_apis`，或项目接口库；不是模板 |
| 只读勘察 / 写流 / 导入接口 | [MCP](./mcp.md)（**不写**模板；导入须开「允许 MCP 导入接口」） |
