...
**12** read-only tools (vs Web AI panel: adds `list_flows` / `get_flow`; no submit tools).
...
- MCP is **read-only**; design changes go through Web **AI assistant** (Diff then merge).
