...
- **其它硬拦**：缺对应端凭证来源 → `AUTH_TOKEN_MISSING`（**仅运行**硬拦；AI `submit_*` / Staging ✓ / **保存**跳过，分批确认时图常不完整）。托管头补全为 soft warning（`AUTH_HEADER_MANAGED`）。
...
| 造流      | 提示集 + 真实 apiId；长流**新对话 + 短提示**；本轮无 Staging 摘要 = 未落盘   |
