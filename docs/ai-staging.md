...
**MCP 默认只读**：无写工具。项目设置开启「允许 MCP 全自动写流」后，可经 MCP 调用 `submit_*` / upsert / `run_test_flow`（每次成功 submit 立即落盘）；接入说明见 [mcp.md](./mcp.md)。MCP 与 Web 全自动落盘共用外部变更同步语义（`graphCommitted` 等）：打开中的画布经 SSE 轻量跟上，脏稿/Staging 时不静默覆盖。详见 [mcp-canvas-live-sync.md](./mcp-canvas-live-sync.md)。
...
| `submit_*` 单元写工具 | **有** | 无 | **有**（每次成功即落盘） |
| `upsert_asset_variables` | **有**（半自动：提案；全自动：直写） | 无 | **有**（直写） |
