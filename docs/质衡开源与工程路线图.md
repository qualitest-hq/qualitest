...
4. **落地页 Pages**：主仓 Settings → Pages → Source = **GitHub Actions** → Actions 跑 `Deploy Pages`（或 `workflow_dispatch`）→ About → Website = `https://qualitest-hq.github.io/qualitest/`（源码 [`site/`](../site/)，步骤见 [`site/README.md`](../site/README.md)）
...
**落地页（GitHub Pages）**：源码在主仓 [`site/`](../site/)；URL `https://qualitest-hq.github.io/qualitest/`。Private 期间不可对外访问。公开日：Settings → Pages → Source = **GitHub Actions**，再跑 `Deploy Pages`（`workflow_dispatch`）；About → Website 填上述 URL。
