...
**约定**：`qualitest` / `qualitest-demo` **不发**空 GitHub Release、不维护 CHANGELOG（变更以 git / README / `docs/` 为准）。**例外**：[`qualitest-intellij-plugin`](https://github.com/qualitest-hq/qualitest-intellij-plugin) 发 GitHub Release（附 ZIP）；变更说明写在 Gradle `changeNotes`，仍不维护根目录 CHANGELOG。
...
2. 保护 `main` → Require status checks（如 `backend` / `frontend`；插件仓为 `verify`）
3. README 标明生产须改默认口令与 `token.secret`
4. **落地页 Pages**：主仓 Settings → Pages → Source = **GitHub Actions** → Variables 设 `ENABLE_PAGES_DEPLOY=true` → Actions 跑 `Deploy Pages` → About → Website = `https://qualitest-hq.github.io/qualitest/`（源码 [`site/`](../site/)，步骤见 [`site/README.md`](../site/README.md)）
5. 插件仓打首个 tag `v1.0.0`（触发 Release；公开前勿打）
...
- [ ] 落地页 `https://qualitest-hq.github.io/qualitest/` 可打开（公开日启用 Pages 后）
...
| 2.3 | ~~CI：`docker build` 校验~~ | **已完成**（`ci.yml`：`docker-app` / `docker-web`，只 build 不 push） | — |
| 2.4 | ~~`docker-compose.override.yml.example`~~ | **已完成**（根目录模板；见 `docs/deploy.md`「改端口」） | — |
...
| C 开发热更 | `dev-deps-up` + 宿主机 `mvn` / `pnpm dev` | ✅ |
...
| `qualitest-intellij-plugin` | 独立 semver（与主仓版本号无关）；IDE 大版本后 1～2 周跟进；**GitHub Release 附 ZIP**；`changeNotes` 在 Gradle；Marketplace 阶段 3.1 后 CI 自动发布。**Private / 公开前勿打 tag**；公开日再打首个 `v1.0.0`。修 BUG 升 PATCH（不可同版本覆盖）。详情见插件仓 [README · 发版](https://github.com/qualitest-hq/qualitest-intellij-plugin#发版维护者) |

长期：Issue 分级；首发期后尽量 48h 内回复；每季度过 Dependabot。

**落地页（GitHub Pages）**：源码在主仓 [`site/`](../site/)；URL `https://qualitest-hq.github.io/qualitest/`。Private 期间不可对外访问；`Deploy Pages` 默认只 **build**，设仓库变量 `ENABLE_PAGES_DEPLOY=true` 后才 deploy。公开日：Pages Source = GitHub Actions → 开变量 → 跑 workflow → About Website。
...
- [ ] 公开日：启用 Pages（Source = GitHub Actions）并确认落地页可访问
- [ ] 公开日：插件仓打首个 tag `v1.0.0`（触发 GitHub Release；公开前勿打 tag）
- [ ] 1.3 GIF（≥2）
- [ ] 1.4 首发文发出
- [ ] 验收标准全部通过
...
- [ ] 2.2 GHCR
- [x] 2.3 CI docker build（`docker-app` / `docker-web`，不 push）
- [x] 2.4 override 示例（`docker-compose.override.yml.example`）
...
| [`site/README.md`](../site/README.md) | GitHub Pages 落地页；**公开日上线步骤** |
| [`docs/deploy.md`](./deploy.md) / [`.en.md`](./deploy.en.md) | 部署、生产加固、Flyway、Pages 入口 |
