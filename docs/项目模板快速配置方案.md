...
> 状态：契约已冻结于 `docs/project-template/`；import/export 按阶段 1 落地。  
...
**一句话**：人或 AI 只产出**精简 JSON**（接口/素材/环境）；质衡规则展开入库（**不生成测试流**）；登录流在导入后用画布配置并调试。
...
| 再用初始化 API 让质衡 AI 生成模板 | ⚠️ 改成 **import 落库** + **画布配流** | 结构映射确定性更高；流需调试，不在 import 一次合成 |
...
- import / validate / schema / aiPrompt：`project:testProjectTemplate:add`
