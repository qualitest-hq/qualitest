...
Same side: login once (or subflow), reuse managed Bearer on `asset.*` (probe-then-login: whitelist `[200,401,403]` + Condition `http.status=200` and `http.expectedMatch`; API `expectedResponseKind` default json). Dual-side same graph: two logins, two extracts — never overwrite one credential path (`AUTH_LOGIN_FLOWKEY_COLLISION`). Missing source → `AUTH_TOKEN_MISSING` (**Run** hard-block; unit `submit_*` → warnings; Staging ✓ / Save soft; may tip “wrong side” Profile). Soft: `AUTH_HEADER_MANAGED`. AI: `list_project_auth_profiles` + `upsert_auth_profile`; next turn may carry `runRiskWarnings`. Details: [ai-staging.en.md](./ai-staging.en.md) · [faq.en.md](./faq.en.md) · [project-template.en.md](./project-template.en.md).
