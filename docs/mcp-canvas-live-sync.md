...
| 路径 | 写库时机 | Web 如何感知 |
| ---- | -------- | ------------ |
| Web AI **半自动** | Staging ✓ → 人手保存 | 本地已是提案图，保存后一致；Hub 亦发 `graphCommitted`（本端保存回声被抑制） |
| Web AI **全自动** | 隐式落盘后 SSE `graphCommitted` + Hub | 清 Staging + 轻量 `loadFlow`（与画布订阅去重） |
| **MCP 全自动写流** | `submit_*` / upsert / `run_test_flow` 成功即写库 | 打开中画布经 `GET .../events` 订阅 Hub，按域同步 |
...
| **事件内联整图** | — | **暂不做**（通道膨胀） |
| **增量 patch apply** | **已实现**：Autopilot/MCP 落盘带 `changedNodeIds` + `nodePatches` / `edgePatches` | 合并进 store 再高亮；校验失败或结构异常 → `loadFlow` 兜底 |

无论哪种，同步入口必须是轻量 API（如 `syncFlowFromServer` / `applyExternalGraph`），**禁止**调用 `initFlow`。

人手 Web 保存仍发无 patch 的 `graphCommitted`，前端走 `loadFlow`（本端回声被抑制）。
...
- [x] 连续 10 次 MCP `submit_*`，打开中画布 debounce 窗口内至多 1～2 次拉图/应用图。
- [x] 纯 `graphCommitted` 同步不出现 env / runs / apiHealth / savePrecheck 必发请求。
- [x] 素材-only / 鉴权-only 事件不触发 `loadFlow`。
...
| 方案 | 优点 | 代价 |
| ---- | ---- | ---- |
| **按流/项目订阅 SSE**（已实现） | 实时、事件类型完整（图/素材/鉴权/Run） | 端点、重连、鉴权 |
| **短轮询**元数据 | 落地快 | 延迟；多域需多个版本戳或聚合接口 → **暂不做** |
| **Redis Pub/Sub → 推到已连接会话**（已实现） | 多实例 | 会话注册 |

**推荐形态**：产品化以 **SSE 长订阅** 为主 + Redis 扇出；**轮询降级暂不做**。
...
- **只锁写路径**，打开浏览不占锁。
- **先到先得**（Redis `SET NX` + TTL + 心跳）：Web 脏稿长持锁（`POST .../editLease` + 约 20s heartbeat）；保存带 `X-Flow-Edit-Lease` 续期不换锁；MCP / 无租约请求短抢短释。任一持锁则另一方写失败并返回明确错误（MCP 回执 `lockHeldBy` + `hint` / Web Toast）。
- 粒度：默认 **`testFlowId` 级**（改图）。素材/鉴权 **项目域锁暂不做**（仅用抽屉/参数库打开启发式脏稿条幅，不静默覆盖）。
- 租约：TTL 45s + 持有方心跳；Tab 关闭 / `markClean` 须释放；超时视为可抢。
- 持锁方仍发 `graphCommitted` 等事件；他端若只读可同步画面；他端若有脏稿仍走 §4.4。

### 7.2 与脏稿

未持锁的 Web 若本地 dirty，外部事件只提示拉取；用户「放弃本地并拉取」后可继续看，若要再保存须重新抢锁。

鉴权：项目设置抽屉打开中视为脏；素材：左栏 `params` 打开中视为脏——脏则顶栏条幅「放弃并刷新」/「稍后」，不自动覆盖。
...
- [x] MCP 连续 `submit_*` 改打开中的流：无需手动刷新，结构跟上，新节点有高亮（动画若开启则可见且不狂闪）。
- [x] 有本地 dirty / Staging：不覆盖，可「放弃本地并拉取」。
- [x] 打开流 B 时，流 A 的图事件不误应用到 B。
- [x] Web 全自动 `graphCommitted` 回归通过。

**其它域**

- [x] MCP upsert 素材后，打开中项目的变量/素材视图更新（或脏表单提示），且不强制 `loadFlow`。
- [x] MCP upsert 鉴权后，鉴权配置与节点鉴权展示更新，不强制整页刷新。
- [x] MCP `run_test_flow` 后出现 Run 高亮/列表更新，不因此无故整图重拉。

**锁与性能**

- [x] Web 脏稿持锁 + 心跳；保存带 `X-Flow-Edit-Lease`；MCP 写同流失败回执含 `lockHeldBy` / `hint`。
- [x] 浏览态不占锁；TTL 到期可再抢。
- [x] Autopilot/MCP `graphCommitted` 可带增量片段；merge 失败回退 `loadFlow`。
- [x] §5.2 性能验收通过；校验无按节点 ×N 整图重算。
- [ ] SSE 轮询降级：**暂不做**。
- [ ] 素材/鉴权项目域锁：**暂不做**（启发式脏稿条幅已实现）。

**指引**

- [x] [mcp.md](./mcp.md) / Skill：改为打开对应测试流即可看到同步；说明脏稿与锁冲突时的提示。
- [x] [ai-staging.md](./ai-staging.md)：MCP 与 Web 全自动共用外部变更同步语义。
