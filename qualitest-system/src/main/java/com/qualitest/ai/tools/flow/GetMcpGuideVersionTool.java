...
/**
 * MCP 只读工具：查询当前项目的合成规程版本（源文指纹 + 可选写流/导入后缀）及使用提示。
 */
...
public class GetMcpGuideVersionTool implements QualitestTool {
    ...
    /**
     * 执行工具：根据调用上下文中的项目 id 读出两道开关，返回合成 guideVersion 与 hint。
    ...
    public String execute(Map<String, Object> arguments, FlowDesignToolContext ctx) {
        ...
        String version = mcpPromptResourceService.guideVersion(
                gates.autopilotEnabled(), gates.importApisEnabled());
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("guideVersion", version);
        body.put("hint", "将返回值与业务仓 .cursor/skills/qualitest/SKILL.md 中的 guideVersion 做字符串全等比较；"
                + "一致则勿再 prompts/get 规程正文。不一致（含改写流/导入开关导致 +autopilot/+importApis 后缀变化）时执行 qualitest_sync_local_skill。"
                + "格式为 {规程指纹}[+autopilot][+importApis]。");
