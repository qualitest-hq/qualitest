...
<!-- mcp:autowrite -->
2. **写流 / 新建流**前：质衡「项目设置」已开启并保存「允许 MCP 自动写流」。保存后请重连或刷新 MCP，否则编辑器常仍只列只读工具。
3. **改图必须带已有 `testFlowId`**。没有则先 `list_flows`；仍没有合适流则用 `create_flow` 新建空画布后再继续。
<!-- /mcp:autowrite -->
<!-- mcp:autorun -->
4. **自动跑流**前：另开「允许 MCP 自动跑流」。只开写流不能调用 `run_test_flow`。保存后请重连 MCP 并再 sync Skill。
<!-- /mcp:autorun -->
<!-- mcp:import -->
5. **导入接口**前：另开「允许 MCP 导入接口」。只开导入不能改图；只开写流不能调用 `import_apis`。保存后请重连 MCP 并再 sync Skill。
<!-- /mcp:import -->
