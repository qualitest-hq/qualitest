...
name: qualitest
description: >-
  经质衡（QualiTest）MCP 勘察、改测试流并跑通。在用户提到质衡 MCP、造流、修流、跑测试流、
  submit、create_flow、import_apis、run_test_flow、项目 Token、允许 MCP 全自动写流、
  允许 MCP 导入接口，或要在 Cursor 里直接改质衡画布时使用。
---

# 质衡 MCP 造流 / 修流

通过已配置的 `qualitest` MCP 操作当前 Token 绑定的测试项目。业务仓代码只作上下文；改图画布与跑流一律走 MCP 工具。

## 前置

1. Cursor 已配置质衡 MCP（`mcp.json` + `X-Project-Token`）。Token 只绑定项目身份，不区分读写。
2. **写流 / 跑流 / 新建流**前：质衡「项目设置」已开启并保存「允许 MCP 全自动写流」。未开则只能只读勘察（及可选导入）。开启后若仍只见只读工具，请重连或刷新 MCP。
3. **导入接口**前：另开「允许 MCP 导入接口」。只开导入不能改图；只开写流不能调用 `import_apis`。
4. **改图必须带已有 `testFlowId`**。没有则先 `list_flows`；仍没有合适流则用 `create_flow` 新建空画布后再继续。

## 硬规矩

- **每次成功的 `submit_*` 已立即写库**。不要找 commit / 保存工具，也不要假设还要 Staging 确认。
- **`import_apis` 成功即写接口库**；不会改写接口上的设计提示字段（设计提示用专门的追加工具写入）。
- 用户若在 Web **打开了同一测试流**，画布会自动跟上 MCP 落盘（无需提醒「请刷新」）。若对方有未保存脏稿，Web 会提示冲突，不是 MCP 侧失败。
- 造流、修流、修失败、用户要「跑通/验证」时：必要 `submit_*`（及缺省时的 upsert）完成后 **立刻** `run_test_flow`，不要只改图就结束。
- 若工具报「正在被其它端写入」：等待数秒后重试，或换一流再写。
- 纯答疑（解释节点/字段、只要建议不改库）时：只用只读工具；不要 `submit_*` / `create_flow` / `import_apis` / `run_test_flow`。
- 不要输出整份 `graphJson`；拓扑用 `get_graph_summary` / `get_node_detail`。
- 用户要看图时：把 `get_graph_summary` 返回的 `mermaid` 原样放进 ` ```mermaid ` 代码块，勿手搓图。
- 不要编造接口 path / 测值；先 `search_apis` → `get_api_details`；库无目标时从业务仓抽 → `import_apis` → `get_api_details` → `submit_http_node(callMode=project)`。测值从业务仓或素材键读取，读不到标不确定。
- 本 Skill **不是**「可测提示词」路径：不要只汇总短提示贴回 Web；开了写权限就直接 MCP 改流。

## 接口分组 / 注释占位（优先）

先用本占位；skill 缺失时再试读本机 JetBrains `options/qualitest-settings.xml` 的 `groupTag` / `ignoreFirstGroupLevel`（勿泄露 projectToken）。
...
从业务仓 Controller 抽取；有标注则传入覆盖；省略则更新保留库内。看回执 `warnings` / `metaGroupSource`。
...
1. 确认 `testFlowId`；没有合适流时先 `create_flow`，再读相关接口（缺则先 `import_apis`）。
2. 缺素材 / 鉴权时先 `upsert_asset_variables` / `upsert_auth_profile`（调用即写库）。
3. 每次只调 **一个** `submit_*` 单元；HTTP 优先 `callMode=project` + `testProjectApiId`。
...
| 工具 | 要点 |
|------|------|
| `import_apis` | 须「允许 MCP 导入接口」；结构化 items 幂等 upsert；成功即落库 |
